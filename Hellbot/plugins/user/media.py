...
async def mediaInfo(_, message: Message):
    ...
    to_paste = f"<strong>💫 AujlaBot Media Info:</strong><br>{metadata}<br><b>📝 MediaInfo:</b><br><code>{out}</code>"
    link = post_to_telegraph("AujlaBotMediaInfo", to_paste)
...
async def memify(_, message: Message):
    ...
    await message.reply_photo(
        memes[0],
        caption=f"**🍀 𝖬𝖾𝗆𝗂𝖿𝗂𝖾𝖽 𝗎𝗌𝗂𝗇𝗀 AujlaBot!**",
    )
